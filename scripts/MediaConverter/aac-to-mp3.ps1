...
foreach ($file in $fileList) { $filename = $file.BaseName; ffmpeg -i $filePath\\$filename.aac $filePath\\$filename.mp3} # for every file, output copy of aac to mp3 to its own file in the same directory