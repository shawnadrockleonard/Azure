...
param (
    ...
    [Parameter(Mandatory = $false)]
    [string]$AZMasterVMname = "Win10EntMulti",

    [Parameter(Mandatory = $false)]
    [string]$AZResourceGroup = "Infrastructure",

    [Parameter(Mandatory = $true, HelpMessage = "Identity the subscription into which the image will be provisioned.")]
    [string]$AZSubscriptionID,

    [Parameter(Mandatory = $false)]
    [string]$vmNewName = "1909-multi-o365",

    [Parameter(Mandatory = $false, HelpMessage = "Flag to indicate its running in Azure Government")]
    [System.Management.Automation.SwitchParameter]$ISGov
)
BEGIN
{
    #################################
...
PROCESS
{
    ...
    #
    #	Start-Process -FilePath C:\Windows\System32\Sysprep\Sysprep.exe -ArgumentList â€˜/generalize /oobe /shutdown /quietâ€™
    #
    #################################
    break
    ...
    if ( get-azgallery | Where-Object { $_.Name -eq $ImageGalleryName } )
    {
        "Gallery Found"
    ...
    if (Get-AzGalleryImagedefinition -ResourceGroupName $AZResourceGroup -GalleryName $ImageGalleryName | Where-Object { $_.Name -eq $ImageName })
    { 
    ...
    $TempVM = get-azvm -name $vmNewName
    $ImageVersion = '1.2.0'
