...
try
{
    ...
    $taggedResourceGroups = @(Get-AzureRmResourceGroup | Where-Object { $_.Tags.Count -gt 0 -and $_.Tags.Keys -contains "ShutdownStartup-Test-A" })
    ...
    foreach ($vm in $resourceManagerVMList)
    {
        ...
        if ($vm.Tags.Keys -contains "ShutdownStartup-Test-A")
        {
