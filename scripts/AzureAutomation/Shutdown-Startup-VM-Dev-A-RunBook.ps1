...
ShutdownStartup-Dev-A = VM part of a resource group (-D) or (Dev)


Tag value is ShutdownStartup-Test-A
Tag value was AutoShutdownSchedule

ShutdownStartup-Test-A = VM part of a test resource groups (-T) or (Test) and or Test VMs.
    ...
    .DESCRIPTION
        ...
        virtual machines or resource groups with a tag named "ShutdownStartup-Dev-A" having a value defining the schedule, 
...
Select-AzureRmSubscription -SubscriptionId $subscriptionObj.Id
...
try
{
    ...
    foreach ($vm in $resourceManagerVMList)
    {
        ...
        if ($vm.Tags.Keys -contains "ShutdownStartup-Dev-A")
        {
