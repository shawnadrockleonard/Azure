...
foreach ($file in $fileList) { $filename = $file.BaseName; ffmpeg -i $filePath\\$filename.mp4 -vn -acodec copy $filePath\\$filename.aac} # for every file, copy out the aac stream to its own file in the same directory