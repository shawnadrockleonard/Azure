configuration ConfigSFCI
{
    ...
    (
		...
		[Parameter(Mandatory)]
        [System.Management.Automation.PSCredential]$sqlLoginCreds,
    ...
    Service MsmqService
    {
        Name = 'MSMQ'
        State = 'Running'
        DependsOn = '[WindowsFeature]MSMQ-Directory'
    }
        ...
        Script EnableS2D {
            SetScript  = "Enable-ClusterS2D -Confirm:0; New-Volume -StoragePoolFriendlyName S2D* -FriendlyName QVDisk -FileSystem NTFS -DriveLetter ${qDriveLetter} -Size 40GB; New-Volume -StoragePoolFriendlyName S2D* -FriendlyName VDisk01 -FileSystem NTFS -DriveLetter ${driveLetter} -UseMaximumSize"
        ...
        xSQLServerFailoverClusterSetup PrepareMSSQLSERVER
        {
            ...
            FailoverClusterNetworkName = $clusterNetworkName
        ...
        xFirewall SQLFirewall
        {
            ...
            Direction   = "Inbound"
            RemotePort  = "Any"
            LocalPort   = ("445", $svcPort.ToString(), $probePort.ToString(), ($probePort + 1).ToString())
            Protocol    = "TCP"
		...
		xSQLServerLogin Add_SqlLogin
        {
            ...
            Ensure                         = 'Present'
            Name                           = $sqlLoginCreds.UserName
            LoginType                      = 'SqlLogin'
            SQLServer                     = $SQLClusterName
            SQLInstanceName                   = "MSSQLSERVER"
            LoginCredential                = $sqlLoginCreds
            PsDscRunAsCredential           = $DomainCreds
        }

		xSQLServerRole Add_SqlLoginToRole
        {
            DependsOn                      = "[xSQLServerLogin]Add_SqlLogin"
            Ensure                         = 'Present'
            Name                           = $sqlLoginCreds.UserName
            ServerRole                     = 'sysadmin'
            SQLServer                      = $SQLClusterName
