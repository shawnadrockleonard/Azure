...
{
  [Cmdlet("Connect", "AzureCMADALPassword")]
  [CmdletHelp("Walks through connecting to Azure AD for tokens", Category = "Base Cmdlets")]
  public class ConnectAzureCMADALPassword : AzureCmdlet
  {
    ...
    [Parameter(Mandatory = true, HelpMessage = "The ClientId of the resource to query")]
    public string ClientId { get; set; }

    [Parameter(Mandatory = true, HelpMessage = "The TenantId of the resource to query")]
    ...
    protected override void ProcessRecord()
    {
      ...
      App = PublicClientApplicationBuilder.CreateWithApplicationOptions(config)
                                              .WithAuthority(config.AzureCloudInstance, config.AadAuthorityAudience)  // work around to MSAL.NET bug #969
                                              .Build();

      var tokens = AcquireATokenFromCacheOrUsernamePasswordAsync(new string[] { "User.Read", "User.ReadBasic.All" }).GetAwaiter().GetResult();
    }
    ...
    public async Task<AuthenticationResult> AcquireATokenFromCacheOrUsernamePasswordAsync(IEnumerable<string> scopes)
    {
    ...
    private async Task<AuthenticationResult> GetTokenForWebApiUsingUsernamePasswordAsync(IEnumerable<string> scopes, string username, SecureString password)
    {
      ...
      {
        System.Diagnostics.Trace.TraceError($"MsalUiRequiredException {ex}");
      ...
      {
        System.Diagnostics.Trace.TraceError($"MsalServiceException {ex}");
        // ------------------------------------------------------------------------
      ...
      {
        System.Diagnostics.Trace.TraceError($"MsalClientException {ex}");
