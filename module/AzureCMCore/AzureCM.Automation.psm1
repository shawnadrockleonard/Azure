...
Function New-AzureCMVirtualMachine {
	...
	process {
		...
		if (!(Get-AzureVM -ServiceName $service -Name $vm.name)) {
			...
			$vmImage = Get-AzVMImage | where { $_.ImageFamily -eq $vm.imageFamily } | sort PublishedDate -Descending | select -ExpandProperty ImageName -First 1
			$environment = Get-AzEnvironment | Where-Object Name -eq $azureEnvironment
			...
			$storagekey = Get-AzStorageAccountKey -StorageAccountName $storage
			...
			if ($availabilitySet -ne $null -and $availabilitySet.length -gt 1) {
				...
				$vmConfig = New-AzVMConfig -Name $vm.name -InstanceSize $vm.size -ImageName $vmImage -MediaLocation $osDiskName -AvailabilitySetName $availabilitySet `
				| Set-AzSubnet $vm.subnet `
				| Set-AzStaticVNetIP -IPAddress $vm.ip

			}
			else {

				$vmConfig = New-AzVMConfig -Name $vm.name -InstanceSize $vm.size -ImageName $vmImage -MediaLocation $osDiskName `
				| Set-AzSubnet $vm.subnet `
