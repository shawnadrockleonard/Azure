...
Configuration CreateDomainController {
    ...
    Node localhost
    {
        ...
        # Add "adfs" to DNS server
        Script SetDnsServerAddressToFindADFS
        {
            GetScript = {return @{}}
            ...
            SetScript = 
            {
                Write-Verbose -Message 'Adding adfs DNS A record.';
                Add-DnsServerResourceRecordA -Name $using:HostName -ZoneName $using:DomainName -AllowUpdateAny -IPv4Address $using:HostIp

                Write-Verbose -Message 'Adding enterpriseregistration DNS CNAME record.';
